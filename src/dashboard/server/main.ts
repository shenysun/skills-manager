...
export type { DetectionStatus, DetectionOutcome } from '../../core/services/detection-service.js';
...
export function createDashboardApp(options: DashboardServerOptions): FastifyInstance {
  ...
  // One detection service per app (not per request): it owns the remote-head
  // TTL cache, and the GitHub client it holds owns the gh-CLI probe verdict and
  // the per-owner/repo@ref TTL cache, so a refreshed page hits memory, not the
  // API. getServices() rebuilds services every request — neither may live
  // there (ADR-0013 detection pipeline).
  ...
  const state = async () => {
    ...
    const updatable = await detection.detect(services, listed);
